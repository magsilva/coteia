...
if ( $get_swiki[0] == "" ) {
  ...
  if ( $atualiza=="0" ) {
		header( "Location:setswiki.php" ); //Redireciona para interface anterior
	}
} else {
...
<table border="1" cellspacing="0" cellpadding="5" class="box-table">
	...
	<tr>
		...
		<td><input type="text" name="new_swiki" size="25" maxlength="70" value="<?php echo $titulo?>"></td>
	</tr>
	<tr>
		<td valign="middle">Alias</td>
		<td><input type="text" name="alias" size="25" maxlength="70" value="<?php echo $alias?>"></td>
	</tr>
		<tr><td valign="middle">Administrador</td>
		<td><input type="text" name="admin" size="25" maxlength="40" value="<?php echo $admin?>"></td>
	</tr>
	<tr>
		<td valign="middle">Email</td>
		<td><input type="text" name="admail" size="25" maxlength="50" value="<?php echo $admin_mail?>"></td>
	</tr>
		<tr><td valign="middle">Login (opcional)</td>
		<td><input type="text" name="usuario" size="25" maxlength="10" value="<?php echo $username?>"></td>
	</tr>
	<tr>
		<td valign="middle">Senha (opcional)</td>
		<td><input type="password" name="passwd" size="25" maxlength="10" value="<?php echo $senha?>"></td>
	</tr>
	<tr>
		<td valign="middle">Status</td>
		<td>
			<select name="status">
				<option value="0" <?php if ( $status == "0" ) echo " selected" ?>>Padrão</option>
				<option value="1" <?php if ( $status == "1" ) echo " selected" ?>>Login</option>
			</select>
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td>
			<select name="sem">
				<option value="T" <?php if ($semestre =="T") echo " selected"; ?>>Todos</option>
				<?php
        $today = getdate();
        $year = $today['year'];
        $year_ini = $year - 2;
	...
	<tr>
		...
		<td>
			<select name="ann_log">
        <option value="S" <?php if ( $annLog == "S" ) echo " selected" ?>>Sim</option>
        <option value="N" <?php if ( $annLog == "N" ) echo " selected" ?>>Não</option>
	    </select>
		</td>
	</tr>
	<tr>
	...
	<tr>
    <td valign="middle" colspan="2" class="table-footer">	<a href="main.php">Menu Principal</a></td>
	</tr>
	<input type="hidden" name="parametro" value="3" /> 
	<input type="hidden" name="atualiza" value="<?php echo $atualiza?>" /> 
</table>
</form>
