...
if ( $action == "retrieve" ) {
	...
	$revision = cvs_check_revision( $MODULE, $filename, $_REQUEST[ "revision" ] );
	if ( $revision === false ) {
...
if ( $action == "list" ) {
	...
	*/
	$filename = wikipage2cvs( $wikipage_id );
	$revisions = cvs_get_revisions( $CVS_MODULE, $filename );
?>
